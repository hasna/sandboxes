...
export function registerStorageCommands(program: Command): void {
  const storage = program
    .command("storage")
    .description("Manage sandboxes local/remote storage sync");

  storage
    .command("status")
    .description("Show local database and storage sync status")
    .option("--check", "Check remote PostgreSQL connectivity when configured")
    .option("--json", "Output as JSON")
    .action(async (opts) => {
      const status = opts.check ? await getStorageStatusWithRemoteCheck() : getStorageStatus();
      if (opts.json) {
        printJson(status);
        return;
      }
      ...
      if (status.remote) {
        console.log(`Remote: ${status.remote.checked ? (status.remote.ok ? "ok" : "error") : "not checked"}`);
        if (status.remote.error) console.log(`  ${status.remote.error}`);
      }
  ...
  storage
    .command("push")
    .description("Push local sandboxes data to PostgreSQL")
    ...
    .action(async (opts) => {
      try {
        ...
        if (hasSyncBatchErrors(results)) process.exitCode = 1;
      } catch (error) {
  ...
  storage
    .command("pull")
    .description("Pull PostgreSQL sandboxes data into the local database")
    .option("--tables <tables>", "Comma-separated table names")
    .option("--json", "Output as JSON")
    .action(async (opts) => {
      try {
  ...
  storage
    ...
    .action(async (opts) => {
      try {
        ...
        if (hasSyncBatchErrors(result)) process.exitCode = 1;
      } catch (error) {
        if (opts.json) printJson({ error: error instanceof Error ? error.message : String(error) });
        else console.error(chalk.red(error instanceof Error ? error.message : String(error)));
        process.exitCode = 1;
  ...
  storage
    ...
    .action(async (opts) => {
      try {
        ...
        const result = await applyPgMigrations(opts.connectionString || getStorageConnectionString("sandboxes"));
        if (opts.json) {
          printJson(result);
          return;
        }
